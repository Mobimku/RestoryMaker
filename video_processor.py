...
def _apply_final_effects(input_path, output_path, user_settings, **kwargs):
    ...
    command = f'ffmpeg {inputs} -filter_complex "{filter_complex}" -map "{video_stream}" -map "{audio_stream}" -c:v libx264 -preset medium {output_path}'
    return run_ffmpeg_command(command, **kwargs)
...
def _process_segment(segment_data, vo_audio_path, source_video_path, work_dir, stop_event, **kwargs):
    ...
    kwargs['progress_callback'](f"--- Starting processing for segment: {segment_label} ---")
    ...
    raw_clips_dir = segment_work_dir / "raw_clips"
    ...
    effected_clips_dir = segment_work_dir / "effected_clips"
    ...
    for i, clip_path in enumerate(raw_clips):
        ...
        selected_effects = random.sample(effects_pool, num_effects) if num_effects > 0 else []
    ...
    video_duration = get_duration(seg_joined_path)
    if not vo_duration or not video_duration: return None

    final_segment_path = work_dir / f"seg_{segment_label.lower()}.mp4"
    command = (f'ffmpeg -i \"{seg_joined_path}\" -i \"{vo_audio_path}\" -filter_complex "[0:v]setpts=({vo_duration}/{video_duration})*PTS[v]" '
               f'-map "[v]" -map 1:a -c:v libx264 -preset medium -crf 23 -c:a aac -b:a 192k \"{final_segment_path}\"')
...
def process_video(storyboard: dict, source_video_path: str, vo_audio_map: dict, user_settings: dict, stop_event: threading.Event, progress_callback=None):
    """The main function to orchestrate the entire video editing process."""
    ...
    try:
        ...
        if len(processed_segment_paths) > 1:
            ...
            # Using simple concat for final assembly as crossfade is very complex
            command = f"ffmpeg -f concat -safe 0 -i \"{concat_list_path}\" -c copy \"{concat_video_path}\""
            if not run_ffmpeg_command(command, **kwargs):
                raise Exception("Final concatenation failed.")
        else:
            shutil.copy(processed_segment_paths[0], concat_video_path)
