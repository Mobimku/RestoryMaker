...
class App(ctk.CTk):
    ...
    def _start_processing(self):
        ...
        if not all([mp4, srt, self.output_folder.get(), api_key_to_use]):
            self.log_message("ERROR: Silakan pilih file MP4, SRT, Folder Output, dan tambahkan API Key."); return
    ...
    def _add_api_key(self):
        ...
        if new_key:
            if self.api_manager.add_key(new_key): self.log_message("API Key added successfully."); self._refresh_api_key_list(); self.new_api_key_entry.delete(0, 'end')
    ...
    def _delete_api_key(self):
        ...
        if key_to_delete:
            if messagebox.askyesno("Confirm Delete", f"Are you sure you want to delete the key ending in ...{key_to_delete[-4:]}?"):
                self.api_manager.delete_key(key_to_delete); self.log_message("API Key deleted."); self._refresh_api_key_list()
    ...
    def _clear_api_keys(self):
        if messagebox.askyesno("Confirm Clear All", "Are you sure you want to delete ALL saved API keys? This action cannot be undone."):
            self.api_manager.clear_all_keys(); self.log_message("All API keys have been cleared."); self._refresh_api_key_list()
