...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.mp4_path = ctk.StringVar()
        self.srt_path = ctk.StringVar()
        self.output_folder = ctk.StringVar()
        self.bgm_path = ctk.StringVar()
        self.selected_api_key = ctk.StringVar()
        ...
        self.api_tab = self.tab_view.add("API Management")
        self.tutorial_tab = self.tab_view.add("Tutorial")
    ...
    def setup_editor_tab(self):
        self.editor_tab.grid_columnconfigure(0, weight=3)
        self.editor_tab.grid_columnconfigure(1, weight=2)
        self.editor_tab.grid_rowconfigure(1, weight=1)

        left_col = ctk.CTkFrame(self.editor_tab)
        left_col.grid(row=0, column=0, rowspan=3, padx=10, pady=10, sticky="nsew")
        left_col.grid_rowconfigure(1, weight=1)

        right_col = ctk.CTkFrame(self.editor_tab)
        right_col.grid(row=0, column=1, rowspan=3, padx=10, pady=10, sticky="nsew")

        file_io_frame = ctk.CTkFrame(left_col)
        file_io_frame.pack(padx=10, pady=10, fill="x")
        file_io_frame.grid_columnconfigure(1, weight=1)
        ctk.CTkLabel(file_io_frame, text="MP4 Video File").grid(row=0, column=0, **self.grid_opts("w"))
        ctk.CTkEntry(file_io_frame, textvariable=self.mp4_path, state="disabled").grid(row=0, column=1, **self.grid_opts("ew"))
        ctk.CTkButton(file_io_frame, text="Browse...", command=self._select_mp4_file).grid(row=0, column=2, **self.grid_opts())
        ctk.CTkLabel(file_io_frame, text="SRT Subtitle File").grid(row=1, column=0, **self.grid_opts("w"))
        ctk.CTkEntry(file_io_frame, textvariable=self.srt_path, state="disabled").grid(row=1, column=1, **self.grid_opts("ew"))
        ctk.CTkButton(file_io_frame, text="Browse...", command=self._select_srt_file).grid(row=1, column=2, **self.grid_opts())
        ctk.CTkLabel(file_io_frame, text="Output Folder").grid(row=2, column=0, **self.grid_opts("w"))
        ctk.CTkEntry(file_io_frame, textvariable=self.output_folder, state="disabled").grid(row=2, column=1, **self.grid_opts("ew"))
        ctk.CTkButton(file_io_frame, text="Browse...", command=self._select_output_folder).grid(row=2, column=2, **self.grid_opts())

        self.log_box = ctk.CTkTextbox(left_col, state="disabled", wrap="word")
        self.log_box.pack(padx=10, pady=10, fill="both", expand=True)

        progress_frame = ctk.CTkFrame(left_col)
        ...
        self.start_button = ctk.CTkButton(progress_frame, text="Start Processing", command=self._start_processing)
        self.start_button.grid(row=0, column=1, padx=10, pady=10)
        self.stop_button = ctk.CTkButton(progress_frame, text="Stop", command=self._stop_processing, state="disabled")
        self.stop_button.grid(row=0, column=2, padx=10, pady=10)

        canvas_frame = ctk.CTkFrame(right_col)
        canvas_frame.pack(padx=10, pady=10, fill="x")
        ctk.CTkLabel(canvas_frame, text="Canvas Settings", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=10)
        self.bars_check = ctk.CTkCheckBox(canvas_frame, text="Add Black Bars")
        self.bars_check.pack(anchor="w", padx=10, pady=5)
        self.bars_slider = ctk.CTkSlider(canvas_frame, from_=0, to=200)
        self.bars_slider.pack(fill="x", padx=10, pady=5)
        self.bars_slider.set(0)

        audio_frame = ctk.CTkFrame(right_col)
        audio_frame.pack(padx=10, pady=10, fill="x", pady_=(20, 10))
        ...
        self.main_vol_slider = ctk.CTkSlider(audio_frame, from_=0, to=300)
        self.main_vol_slider.pack(fill="x", padx=10, pady=5)
        self.main_vol_slider.set(100)
        ctk.CTkButton(audio_frame, text="Add Background Music...", command=self._select_bgm_file).pack(fill="x", padx=10, pady=10)
        self.bgm_label = ctk.CTkLabel(audio_frame, text="No BGM file selected.", text_color="gray", wraplength=250)
        self.bgm_label.pack(anchor="w", padx=10)
        ctk.CTkLabel(audio_frame, text="BGM Volume (%)").pack(anchor="w", padx=10, pady=(10, 0))
        self.bgm_vol_slider = ctk.CTkSlider(audio_frame, from_=0, to=100)
        self.bgm_vol_slider.pack(fill="x", padx=10, pady=5)
        self.bgm_vol_slider.set(10)
    ...
    def _select_bgm_file(self):
        ...
        if path:
            ...
            self.bgm_label.configure(text=os.path.basename(path), text_color="white")
        else:
            self.bgm_path.set("")
            self.bgm_label.configure(text="No BGM file selected.", text_color="gray")
    ...
    def _start_processing(self):
        ...
        if not all([mp4, self.srt_path.get(), self.output_folder.get(), api_key_to_use]):
            self.log_message("ERROR: Please select MP4, SRT, Output Folder and add at least one API Key before starting.")
            return
        ...
        self.processing_thread = threading.Thread(target=self._processing_thread_target, args=(film_duration_sec, api_key_to_use))
    ...
    def _processing_thread_target(self, film_duration_sec, api_key):
        try:
            ...
            vo_audio_map = {}
            temp_audio_dir = Path(self.output_folder.get()) / "temp_audio"
    ...
    def setup_api_tab(self):
        ...
        add_frame = ctk.CTkFrame(self.api_tab)
        add_frame.pack(padx=10, pady=10, fill="x")
        ctk.CTkLabel(add_frame, text="New API Key:").pack(side="left", padx=10)
        self.new_api_key_entry = ctk.CTkEntry(add_frame, width=400, show="*")
        self.new_api_key_entry.pack(side="left", fill="x", expand=True, padx=10)
        ...
        mgmt_frame = ctk.CTkFrame(self.api_tab)
        mgmt_frame.pack(padx=10, pady=10, fill="x")
