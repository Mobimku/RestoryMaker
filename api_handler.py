...
def get_storyboard_from_srt(srt_path: str, api_key: str, film_duration: int, output_folder: str, language: str = "en", progress_callback=None):
    """
    Uploads an SRT file, constructs a prompt, and returns the storyboard JSON from Gemini.
    """
    def log(msg):
    ...
    try:
        ...
        # Robust check for empty or blocked response
        if not response.candidates:
            log(f"ERROR: Prompt was blocked by the API. Feedback: {response.prompt_feedback}")
            log("Please check the input file for content that might violate safety policies.")
            return None

        candidate = response.candidates[0]
        # Using the enum name is more robust than the integer value
        if candidate.finish_reason.name != "STOP":
            log(f"ERROR: API generation finished with an unexpected reason: {candidate.finish_reason.name}")
            if candidate.finish_reason.name == "MAX_TOKENS":
                log("The model ran out of space to generate the full response. The input SRT might be too long.")
            elif candidate.finish_reason.name == "SAFETY":
                log("The response was blocked due to safety concerns. The generated content might have been flagged.")

            # Even if it's not a clean stop, try to get partial text for debugging
            try:
                raw_response_text = response.text
                debug_json_path = pathlib.Path(output_folder) / "storyboard_error_output.txt"
                with open(debug_json_path, "w", encoding="utf-8") as f:
                    f.write(raw_response_text)
                log(f"Saved partial/error response to {debug_json_path}")
            except ValueError:
                log("Could not extract any text from the blocked response.")
            return None
        ...
        if uploaded_file:
            log(f"Deleting uploaded file: {uploaded_file.name}")
